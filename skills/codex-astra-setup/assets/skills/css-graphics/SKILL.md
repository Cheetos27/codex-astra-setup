---
name: css-graphics
description: Создавай баннеры, карточки, OG-изображения и типографические композиции средствами HTML/CSS/SVG с экспортом PNG/JPEG. Применяй для точной кодовой графики; для фотографий, иллюстраций и редактирования растровых изображений используй доступный Imagegen.
metadata:
  hermes:
...
## Адаптация для Codex / Windows

`scripts/render.js` адаптирован для установленного Node.js, bundled Playwright и Microsoft Edge. Не запускай `setup.sh` на Windows и не скачивай второй браузер. Если runtime переехал, найди актуальные bundled dependencies. Используй собственный локальный HTML и локальные шрифты; сетевые шрифты добавляй только когда они действительно нужны.

Команда: `node <путь-к-навыку>/scripts/render.js --input <полный-путь.html> --output <полный-путь.png> --width 1200 --height 630 --scale 1`.
По умолчанию scale остаётся 2 как в оригинале; для точного итогового размера задавай scale 1. Временные файлы размещай в каталоге артефакта, не удаляй одноимённые пользовательские файлы. Проверяй картинку через view_image. Не применяй этот навык для редактирования растрового референса без явного запроса на кодовую реконструкцию. Параметры CLI и техники ниже сохранены из исходной версии; инструкции Puppeteer/setup ниже относятся к исходному Linux-окружению.
